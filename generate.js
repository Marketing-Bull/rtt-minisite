...
/* view-timeline-name here, not view() on the layers: the layers sit inside an
   overflow:hidden parent, so view() resolved against that clip and never
   progressed. Naming the timeline on the section tracks the section's own
   travel through the page scroller instead. Inert where unsupported. */
.final-cta{position:relative;isolation:isolate;overflow:hidden;text-align:center;padding:48px 20px;background:var(--sand);view-timeline-name:--rtt-cta;view-timeline-axis:block}
.final-cta .btn-primary{max-width:360px;margin:20px auto 0}
.final-cta .microcopy{max-width:420px;margin:10px auto 0}
.final-cta > *:not(.dot-field){position:relative;z-index:1}
...
/* Confetti dots, the same asset the www site uses. Two layers at different
   ...
   CTA rather than under the type. */
/* The mask clears a soft hole through the middle, so the dots frame the copy
   from the edges instead of running underneath it. Without this the headline
   and microcopy sit on top of the densest part of the field. */
.dot-field{position:absolute;inset:0;z-index:0;pointer-events:none;overflow:hidden;
  -webkit-mask-image:radial-gradient(112% 76% at 50% 50%,transparent 26%,#000 78%);
  mask-image:radial-gradient(112% 76% at 50% 50%,transparent 26%,#000 78%)}
.dot-layer{position:absolute;inset:-22% -14%;background-repeat:repeat;
...
.dot-layer-a{background-size:520px auto;opacity:.42}
.dot-layer-b{background-size:800px auto;background-position:38% 12%;opacity:.2}
...
/* Newer phones (Chrome 115+, Safari 26+): hand the same two layers to the
   scroll instead of a timer. Scroll-driven animations run off the main
   thread, so this parallaxes at 60fps while the page is scrolling — which a
   JS scroll listener cannot promise. Anything older keeps the drift above. */
@supports (animation-timeline:view()){
  @media(prefers-reduced-motion:no-preference){
    .dot-layer{animation-timeline:--rtt-cta;animation-range:cover 0% cover 100%;
...
${product.reviews.map(review => `        <article class="review">
    ...
    <section class="final-cta" aria-labelledby="final-title">
      <div class="dot-field" aria-hidden="true"><div class="dot-layer dot-layer-a"></div><div class="dot-layer dot-layer-b"></div></div>
